...
# MAGIC %fs ls dbfs:/FileStore/tables/scd2Demo
...
# MAGIC %sql
# MAGIC delete from scd2Demo where pk1=666;
...
# MAGIC set dim1 = 100 where pk1 = 777;
...
# MAGIC %md
# MAGIC ## Need to wait for seven days (168 hours by default) for the invalid files to show up
...
# MAGIC %md
# MAGIC #### it is recommended to not use value less than 168 hours.as in production data we may need older data if something goes wrong.Hence the error
...
df = spark.read.option("versionAsOf",2).table("scd2Demo");
...
# MAGIC %fs
# MAGIC ls dbfs:/FileStore/tables/scd2Demo/

# COMMAND ----------

# MAGIC %md 
# MAGIC ### AS we can see above we have removed all the invalidated files and only kept the latest optimized files.
