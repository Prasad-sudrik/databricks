...
-- MAGIC %fs ls dbfs:/user/hive/warehouse/demo_db.db/fire_calls_tbl

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dbutils.fs.rm("dbfs:/user/hive/warehouse/demo_db.db/fire_calls_tbl/",True)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC def setup():
-- MAGIC         spark.sql("CREATE DATABASE IF NOT EXISTS demo_db")
-- MAGIC
...
-- MAGIC                 .saveAsTable("demo_db.fire_calls_tbl")
-- MAGIC         )
-- MAGIC setup()        
...
describe extended demo_db.fire_calls_tbl
...
delete from demo_db.fire_calls_tbl where CallDate = "2002-01-24"
...
update demo_db.fire_calls_tbl set Delay = int(Delay)
...
DESCRIBE HISTORY demo_db.fire_calls_tbl
...
select * from demo_db.fire_calls_tbl version as of 0 where CallDate = "2002-01-24"
...
VACUUM demo_db.fire_calls_tbl RETAIN 0 HOURS 
...
-- MAGIC %fs ls dbfs:/user/hive/warehouse/demo_db.db/flight_time_tbl/
...
ALTER TABLE demo_db.fire_calls_tbl SET TBLPROPERTIES (
...
ALTER TABLE demo_db.fire_calls_tbl DROP columns(SupervisorDistrict, FirePreventionDistrict)
...
REORG table demo_db.fire_calls_tbl APPLY(PURGE)
...
optimize demo_db.fire_calls_tbl zorder by (Year,CallDate)
